...
import static com.gomap.sdk.location.LocationComponentConstants.ACCURACY_RADIUS_ANIMATION_DURATION;
import static com.gomap.sdk.location.LocationComponentConstants.COMPASS_UPDATE_RATE_MS;
import static com.gomap.sdk.location.LocationComponentConstants.MAX_ANIMATION_DURATION_MS;
import static com.gomap.sdk.location.LocationComponentConstants.TRANSITION_ANIMATION_DURATION_MS;
import static com.gomap.sdk.location.Utils.immediateAnimation;
import static com.gomap.sdk.location.Utils.normalize;
import static com.gomap.sdk.location.Utils.shortestRotation;
...
final class LocationAnimatorCoordinator {
  ...
  void startLocationComponentCirclePulsing(LocationComponentOptions options) {
    cancelAnimator(MapboxAnimator.ANIMATOR_PULSING_CIRCLE);
    MapboxAnimator.AnimationsValueChangeListener listener = listeners.get(MapboxAnimator.ANIMATOR_PULSING_CIRCLE);
    if (listener != null) {
      ...
      animatorArray.put(MapboxAnimator.ANIMATOR_PULSING_CIRCLE, pulsingLocationCircleAnimator);
      playPulsingAnimator();
    }
  }
  ...
  void feedNewZoomLevel(double targetZoomLevel, @NonNull CameraPosition currentCameraPosition, long animationDuration,
    ...
    playAnimators(animationDuration, MapboxAnimator.ANIMATOR_ZOOM);
  }
  ...
  void feedNewTilt(double targetTilt, @NonNull CameraPosition currentCameraPosition, long animationDuration,
    ...
    playAnimators(animationDuration, MapboxAnimator.ANIMATOR_TILT);
  }

  private LatLng getPreviousLayerLatLng() {
    LatLng previousLatLng;
    MapboxAnimator latLngAnimator = animatorArray.get(MapboxAnimator.ANIMATOR_LAYER_LATLNG);
  ...
  private float getPreviousLayerGpsBearing() {
    MapboxFloatAnimator animator = (MapboxFloatAnimator) animatorArray.get(MapboxAnimator.ANIMATOR_LAYER_GPS_BEARING);
  ...
  private float getPreviousLayerCompassBearing() {
    MapboxFloatAnimator animator = (MapboxFloatAnimator) animatorArray.get(MapboxAnimator.ANIMATOR_LAYER_COMPASS_BEARING);
  ...
  private float getPreviousAccuracyRadius() {
    MapboxAnimator animator = animatorArray.get(MapboxAnimator.ANIMATOR_LAYER_ACCURACY);
  ...
  private void updateLayerAnimators(LatLng[] latLngValues, Float[] bearingValues) {
    createNewLatLngAnimator(MapboxAnimator.ANIMATOR_LAYER_LATLNG, latLngValues);
    createNewFloatAnimator(MapboxAnimator.ANIMATOR_LAYER_GPS_BEARING, bearingValues);
  }

  private void updateCameraAnimators(LatLng[] latLngValues, Float[] bearingValues) {
    createNewLatLngAnimator(MapboxAnimator.ANIMATOR_CAMERA_LATLNG, latLngValues);
    createNewFloatAnimator(MapboxAnimator.ANIMATOR_CAMERA_GPS_BEARING, bearingValues);
  }
  ...
  private void updateCompassAnimators(float targetCompassBearing, float previousLayerBearing,
    ...
    createNewFloatAnimator(MapboxAnimator.ANIMATOR_LAYER_COMPASS_BEARING, previousLayerBearing, normalizedLayerBearing);

    float normalizedCameraBearing = Utils.shortestRotation(targetCompassBearing, previousCameraBearing);
    createNewFloatAnimator(MapboxAnimator.ANIMATOR_CAMERA_COMPASS_BEARING, previousCameraBearing, normalizedCameraBearing);
  }

  private void updateAccuracyAnimators(float targetAccuracyRadius, float previousAccuracyRadius) {
    createNewFloatAnimator(MapboxAnimator.ANIMATOR_LAYER_ACCURACY, previousAccuracyRadius, targetAccuracyRadius);
  }
  ...
  private void updateZoomAnimator(float targetZoomLevel, float previousZoomLevel,
    ...
    createNewCameraAdapterAnimator(MapboxAnimator.ANIMATOR_ZOOM, new Float[] {previousZoomLevel, targetZoomLevel}, cancelableCallback);
  }

  private void updateTiltAnimator(float targetTilt, float previousTiltLevel,
                                  @Nullable MapboxMap.CancelableCallback cancelableCallback) {
    createNewCameraAdapterAnimator(MapboxAnimator.ANIMATOR_TILT, new Float[] {previousTiltLevel, targetTilt}, cancelableCallback);
  }
  ...
  private void playPulsingAnimator() {
    Animator animator = animatorArray.get(MapboxAnimator.ANIMATOR_PULSING_CIRCLE);
    if (animator != null) {
      animator.start();
    }
  }
  ...
  private boolean resetCameraLatLngAnimation(@NonNull CameraPosition currentCameraPosition) {
    MapboxLatLngAnimator animator = (MapboxLatLngAnimator) animatorArray.get(MapboxAnimator.ANIMATOR_CAMERA_LATLNG);
    if (animator == null) {
      return false;
    }
    ...
    createNewLatLngAnimator(MapboxAnimator.ANIMATOR_CAMERA_LATLNG, previousCameraTarget, currentTarget);
  ...
  private void resetCameraGpsBearingAnimation(@NonNull CameraPosition currentCameraPosition, boolean isGpsNorth) {
    MapboxFloatAnimator animator = (MapboxFloatAnimator) animatorArray.get(MapboxAnimator.ANIMATOR_CAMERA_GPS_BEARING);
    if (animator == null) {
      return;
    }
    ...
    createNewFloatAnimator(MapboxAnimator.ANIMATOR_CAMERA_GPS_BEARING, previousCameraBearing, normalizedCameraBearing);
  }

  private void resetCameraCompassAnimation(@NonNull CameraPosition currentCameraPosition) {
    MapboxFloatAnimator animator =
      (MapboxFloatAnimator) animatorArray.get(MapboxAnimator.ANIMATOR_CAMERA_COMPASS_BEARING);
    if (animator == null) {
      return;
    }
  ...
  void resetAllLayerAnimations() {
    MapboxLatLngAnimator latLngAnimator = (MapboxLatLngAnimator) animatorArray.get(MapboxAnimator.ANIMATOR_LAYER_LATLNG);
    MapboxFloatAnimator gpsBearingAnimator = (MapboxFloatAnimator) animatorArray.get(MapboxAnimator.ANIMATOR_LAYER_GPS_BEARING);
    MapboxFloatAnimator compassBearingAnimator =
      (MapboxFloatAnimator) animatorArray.get(MapboxAnimator.ANIMATOR_LAYER_COMPASS_BEARING);
    MapboxFloatAnimator accuracyAnimator =
      (MapboxFloatAnimator) animatorArray.get(MapboxAnimator.ANIMATOR_LAYER_ACCURACY);
    ...
    if (latLngAnimator != null && gpsBearingAnimator != null) {
      ...
      createNewLatLngAnimator(MapboxAnimator.ANIMATOR_LAYER_LATLNG, currentLatLng, currentLatLngTarget);
      ...
      createNewFloatAnimator(MapboxAnimator.ANIMATOR_LAYER_GPS_BEARING, currentGpsBearing, currentGpsBearingTarget);

      long duration = latLngAnimator.getDuration() - latLngAnimator.getCurrentPlayTime();

      playAnimators(duration, MapboxAnimator.ANIMATOR_LAYER_LATLNG, MapboxAnimator.ANIMATOR_LAYER_GPS_BEARING);
    }
    ...
    if (compassBearingAnimator != null) {
      ...
      createNewFloatAnimator(MapboxAnimator.ANIMATOR_LAYER_COMPASS_BEARING, currentLayerBearing, currentLayerBearingTarget);
      playAnimators(
        compassAnimationEnabled ? COMPASS_UPDATE_RATE_MS : 0,
        MapboxAnimator.ANIMATOR_LAYER_COMPASS_BEARING);
    }
  ...
  void cancelZoomAnimation() {
    cancelAnimator(MapboxAnimator.ANIMATOR_ZOOM);
  }

  void cancelTiltAnimation() {
    cancelAnimator(MapboxAnimator.ANIMATOR_TILT);
  }

  void cancelAndRemoveGpsBearingAnimation() {
    cancelAnimator(MapboxAnimator.ANIMATOR_LAYER_GPS_BEARING);
    animatorArray.remove(MapboxAnimator.ANIMATOR_LAYER_GPS_BEARING);
  }
