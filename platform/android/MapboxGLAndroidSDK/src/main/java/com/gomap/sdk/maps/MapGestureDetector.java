...
import static com.gomap.sdk.maps.MapboxMap.OnCameraMoveStartedListener.REASON_API_GESTURE;
...
final class MapGestureDetector {
  ...
  private final class ScaleGestureListener extends StandardScaleGestureDetector.SimpleStandardOnScaleGestureListener {
    ...
    ScaleGestureListener(double densityMultiplier, float minimumGestureSpeed, float minimumAngledGestureSpeed,
      ...
      this.scaleVelocityRatioThreshold = MapboxConstants.SCALE_VELOCITY_RATIO_THRESHOLD * densityMultiplier;
    }
    ...
    public boolean onScale(@NonNull StandardScaleGestureDetector detector) {
      ...
      if (quickZoom) {
        ...
        double normalizedDeltaChange = MathUtils.normalize(pixelDeltaChange, 0, screenHeight, 0, MapboxConstants.QUICK_ZOOM_MAX_ZOOM_CHANGE);
        ...
        double zoomBy =
          (Math.log(detector.getScaleFactor()) / Math.log(Math.PI / 2)) * MapboxConstants.ZOOM_RATE * uiSettings.getZoomRate();
    ...
    public void onScaleEnd(@NonNull StandardScaleGestureDetector detector, float velocityX, float velocityY) {
      ...
      long animationTime = (long) ((Math.log((Math.abs(zoomAddition)) + 1 / Math.pow(Math.E, 2)) + 2)
        * MapboxConstants.SCALE_VELOCITY_ANIMATION_DURATION_MULTIPLIER);
    ...
    private double calculateScale(double velocityXY, boolean isScalingOut) {
      double zoomAddition = velocityXY * MapboxConstants.MAX_ABSOLUTE_SCALE_VELOCITY_CHANGE * 1e-4;
      zoomAddition = MathUtils.clamp(zoomAddition, 0, MapboxConstants.MAX_ABSOLUTE_SCALE_VELOCITY_CHANGE);
      if (isScalingOut) {
  ...
  private final class RotateGestureListener extends RotateGestureDetector.SimpleOnRotateGestureListener {
    ...
    RotateGestureListener(float minimumScaleSpanWhenRotating, double densityMultiplier, float angularVelocityMultiplier,
      ...
      this.rotateVelocityRatioThreshold = MapboxConstants.ROTATE_VELOCITY_RATIO_THRESHOLD * densityMultiplier;
    ...
    public void onRotateEnd(@NonNull RotateGestureDetector detector, float velocityX,
      ...
      angularVelocity = MathUtils.clamp(angularVelocity, -MapboxConstants.MAXIMUM_ANGULAR_VELOCITY, MapboxConstants.MAXIMUM_ANGULAR_VELOCITY);
