...
import com.gomap.sdk.utils.Compare;
...
import static com.gomap.sdk.utils.Compare.compare;
  ...
  private EGLConfig chooseBestMatchConfig(@NonNull EGL10 egl, EGLDisplay display, EGLConfig[] configs) {
    class Config implements Comparable<Config> {
      ...
      public int compareTo(@NonNull Config other) {
        int i = Compare.compare(bufferFormat.value, other.bufferFormat.value);
        if (i != 0) {
          return i;
        }

        i = Compare.compare(depthStencilFormat.value, other.depthStencilFormat.value);
        if (i != 0) {
          return i;
        }

        i = Compare.compare(isNotConformant, other.isNotConformant);
        if (i != 0) {
          return i;
        }

        i = Compare.compare(isCaveat, other.isCaveat);
        if (i != 0) {
          return i;
        }

        i = Compare.compare(index, other.index);
        if (i != 0) {
          return i;
        }
